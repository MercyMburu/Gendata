...
#a) District_summary.-- Create the District Summary View

CREATE VIEW district_summary AS
SELECT
    j.village AS district_name,
    j.parent_region AS region_name,
    SUM(b.beneficiaries * 6) AS individual_beneficiaries,
    SUM(b.beneficiaries * 6) / SUM(v.total_population) AS beneficiaries_to_population_ratio
FROM beneficiary_partner_data b
JOIN village_locations v
    ON b.village = v.village
JOIN jurisdiction_hierarchy j
    ON v.village = j.village
WHERE j.level = 'District'
GROUP BY j.village, j.parent_region;
...
SELECT 
    ...
    COUNT(DISTINCT j.parent_region) AS districts_reached
FROM beneficiary_partner_data b
LEFT JOIN jurisdiction_hierarchy j
    ON b.village = j.village
GROUP BY b.partner;
