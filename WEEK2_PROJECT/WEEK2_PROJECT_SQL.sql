...
SELECT pd.stud_name,pd.stud_ID,pd.phone_number,sd.stud_email
...
CREATE OR REPLACE VIEW financial_details_view AS
SELECT *,
       CASE 
           WHEN CAST(sem_fee AS DECIMAL(10,2)) = CAST(fee_paid AS DECIMAL(10,2)) THEN TRUE
           ELSE FALSE
       END AS fee_cleared_status
...
SELECT 
    pdt.national_ID,
    pdt.stud_name
FROM personal_details pdt
JOIN financial_details fdt
    ON pdt.stud_ID = fdt.stud_ID
WHERE fdt.fee_cleared = 1;
...
SELECT current_home_county,COUNT(current_home_county)
FROM school_details
...
ORDER BY sd.secondary_school_county;

#i. Get the percentage of students who set their next_of_kin as Mother vs those that set it as Father. (kin_percentage) 
SELECT
    ROUND(
        (SUM(CASE WHEN next_of_kin_relation = 'Mother' THEN 1 ELSE 0 END) * 100.0) / COUNT(*), 2
    ) AS mother_kin_percentage,
    ROUND(
        (SUM(CASE WHEN next_of_kin_relation = 'Father' THEN 1 ELSE 0 END) * 100.0) / COUNT(*), 2
    ) AS father_kin_percentage
